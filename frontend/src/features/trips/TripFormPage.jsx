...
export default function TripFormPage() {
  ...
  const selectedVehicle = availableVehicles.find(
    (v) => v.id === parseInt(form.vehicle_id),
  );
  const cargoExceeds =
  ...
  const handleSubmit = async (e) => {
    ...
    if (cargoExceeds) {
      toast.error(
        `Cargo exceeds vehicle capacity (${selectedVehicle.max_capacity_kg} kg)`,
      );
      return;
    }
    setSubmitting(true);
      ...
      <div className="pageHeader">
        <div>
          ...
          <p className="pageSubtitle">
            Assign a vehicle and driver to a shipment
          </p>
        </div>
        <button className="secondaryBtn" onClick={() => navigate(ROUTES.TRIPS)}>
          ← Back
        </button>
      </div>
      ...
      <form style={{ maxWidth: 640 }} onSubmit={handleSubmit}>
        ...
        <div className="formRow">
          <div className="formGroup">
            ...
            {cargoExceeds && (
              <span style={{ color: "#f87171", fontSize: 12 }}>
                ⚠ Exceeds vehicle capacity ({selectedVehicle.max_capacity_kg}{" "}
                kg)
              </span>
            )}
          ...
          <div className="formGroup">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="e.g. Electronics, 50 boxes"
            />
          </div>
        </div>
        ...
        <div className="formActions">
          ...
          <button
            ...
            disabled={submitting || cargoExceeds}
          >
