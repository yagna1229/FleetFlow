...
export const fetchDrivers = createAsyncThunk('drivers/fetchAll', async (params = {}) => {
...
export const fetchExpiringLicenses = createAsyncThunk('drivers/fetchExpiring', async (days = 30) => {
    return await apiGet(`/api/v1/drivers/expiring-licenses?within_days=${days}`)
})

export const createDriver = createAsyncThunk('drivers/create', async (data, { rejectWithValue }) => {
    try {
...
export const updateDriver = createAsyncThunk('drivers/update', async ({ id, data }, { rejectWithValue }) => {
    try {
...
const driverSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            .addCase(fetchDrivers.pending, (state) => { state.status = 'loading' })
