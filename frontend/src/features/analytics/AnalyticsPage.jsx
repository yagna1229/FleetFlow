...
function FuelEfficiencyChart({ data }) {
  ...
  const path1 = data
    .map((d, i) => `${i === 0 ? "M" : "L"} ${getX(i)} ${getY(d.val1)}`)
    .join(" ");
  const path2 = data
    .map((d, i) => `${i === 0 ? "M" : "L"} ${getX(i)} ${getY(d.val2)}`)
    .join(" ");
        ...
        {data.map((d, i) => (
          <g key={`n1-${i}`}>
            <circle
              cx={getX(i)}
              cy={getY(d.val1)}
              r={hoverIdx === i && hoverLine === "A" ? 7 : 5}
              style={{
                fill:
                  hoverIdx === i && hoverLine === "A"
              ...
              onMouseEnter={(e) => {
                handleHover(e, d.val1, "Vehicle A");
                setHoverIdx(i);
                setHoverLine("A");
              }}
            ...
            <circle
              cx={getX(i)}
              cy={getY(d.val2)}
              r={hoverIdx === i && hoverLine === "B" ? 7 : 5}
              style={{
                fill:
                  hoverIdx === i && hoverLine === "B"
              ...
              onMouseEnter={(e) => {
                handleHover(e, d.val2, "Vehicle B");
                setHoverIdx(i);
                setHoverLine("B");
              }}
              onMouseLeave={unHover}
              ...
              <g key={`b-${i}`}>
                <rect
                  ...
                  style={{
                    fill:
                      hoverIdx === i
                        ? "var(--primary-200, #bae6fd)"
                        : "var(--primary-100, #cffafe)",
                    stroke:
                      hoverIdx === i
                        ? "var(--secondary-500, #0ea5e9)"
                        : "var(--brand, #06b6d4)",
                    cursor: "pointer",
                    transition: "all 0.2s",
                  }}
                  onMouseEnter={(e) => {
                    handleHover(e, d.name, d.actual_cost);
                    setHoverIdx(i);
                  }}
                  onMouseLeave={() => {
                    setTooltip(null);
    ...
    <div className="analyticsWrap">
      ...
      <div className="analyticsTopKPIs">
        ...
        >
          ...
          <div
            className={`analyticsKpiValue ${aggregate.roi.startsWith("+") ? "positive" : ""}`}
          >
            {aggregate.roi}
          </div>
        </div>
